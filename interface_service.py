...
def nav(active: str) -> str:
    return f'''<nav><a class="{"active" if active == "process" else ""}" href="/">Novo processamento</a>
...
def process_page(result: dict | None = None, video_url: str = "", error: str = "") -> bytes:
    ...
    <div class="section-title">Detector de veículos</div><div class="row"><label>Confiança<input name="conf" type="number" min="0" max="1" step="0.01" value="0.45"></label><label>NMS<input name="nms" type="number" min="0" max="1" step="0.01" value="0.45"></label></div>
    <label>Modelo de veículos<input name="model_path" value="models/vehicles.onnx"></label>
...
def job_page(job_id: str) -> bytes:
    ...
    <p class="meta">{esc(row["created_at"])} · estado: {esc(row["state"])}</p><video class="video" controls src="/media/{esc(row["video_name"])}"></video>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path.startswith("/delete/"):
            ...
            db = database()
            row = db.execute("SELECT video_name FROM jobs WHERE id = ?", (job_id,)).fetchone()
            if row:
        ...
        try:
            ...
            payload = {
                "video": str(video_path), "model_path": form.getfirst("model_path") or "models/vehicles.onnx",
                "conf": number("conf", "0.45"), "nms": number("nms", "0.45"), "imgsz": int(number("imgsz", "640")),
            ...
            db = database()
            db.execute("INSERT INTO jobs VALUES (?, ?, ?, datetime('now','localtime'), ?, ?, ?, ?, ?, ?, ?, ?)", (job_id, original, video_name, result.get("state", "finished"), result.get("vehicles"), result.get("axles"), result.get("frames"), result["processing_seconds"], result["cpu_percent"], json.dumps(payload), result.get("error")))
            db.commit(); db.close()
            self.send_page(process_page(result, f"/media/{video_name}"))
        except Exception as error:
            error_text = f"Falha no processamento: {error}"
            db = database()
            db.execute("INSERT INTO jobs VALUES (?, ?, ?, datetime('now','localtime'), ?, ?, ?, ?, ?, ?, ?, ?)", (job_id, original, video_name, "error", None, None, None, None, None, json.dumps(payload if 'payload' in locals() else {}), error_text))
            db.commit(); db.close()
